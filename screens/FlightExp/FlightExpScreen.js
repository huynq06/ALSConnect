...
import { startOfDay,
  ...
  dateWithSec } from '../../utils/dateHelpers';
    import * as flightsActions from '../../stores/actions/flightsExp'
    import * as flightsImpActions from '../../stores/actions/flights'
    import { useToast } from "react-native-toast-notifications";
const FlightExpScreen = ({navigation, route}) => {
  ...
  const flights = useSelector(state => {
      const loadFlight = [];
      if(searchText===''){
          return  state.flightsExp.flights
      }else{
        state.flights.flightsExp.forEach((element, index)=>{
 ...
 const loadFavouriteFlights =  useCallback(async () => {
  ...
  try {
      await dispatch(flightsImpActions.fetchFavouriteFlights('EXPORT'));
  } catch (err) {
  ...
  /*   dispatch(loadingActions.start({ key: 'fetchFlight' })) */
    try {
        ...
        await dispatch(flightsActions.fetchFlightsExp(code, number, date));
    } catch (err) {
      setError(err.message);
    }finally{
...
//   };
  ...
  const toggleFavoriteHandler = (id) =>{
    dispatch(flightsImpActions.addFavouriteFlights(id,tokenId,'EXPORT'))
...
export default FlightExpScreen;
