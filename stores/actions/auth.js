...
export const updateDetail = (city,fullName,dob,cityParam, district, ward, detail, cccd) => {
  return async (dispatch, getState) => {
    const userId = getState().auth.userId;
    const response = await fetch(
      'http://14.160.23.141:3434/api/Users/updateDetail',
      {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
...
export const resetPassword = email => {
  return async () => {
    const response = await fetch('http://14.160.23.141:3434/api/email/send', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
...
export const signup = (email, user, password) => {
  return async dispatch => {
    const response = await fetch(
      'http://14.160.23.141:3434/api/Users/register',
      {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
